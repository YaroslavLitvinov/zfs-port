...
#ifdef ZVM_MM_DEBUG
	printf("sizeof vmem_freelist_t=%d\n", sizeof(vmem_freelist_t));
	printf("vm_name=%s\tvs_size=%u, realsize=%u, addr=%p, vs_start=%p\n",
	       vmp->vm_name, vs_size, realsize, addr, vs_start);
	fflush(0);
	if ( !strcmp("umem_cache", vmp->vm_name) ){
	    int i;
	    i=2;
	}
#endif //DEBUG
...
{
#ifdef ZVM_MM_DEBUG
	    printf("vmem_reap::vmem_nosleep_lock.vmpl_thr = %u, thr_self()=%u\n",
...
vmem_populate(vmem_t *vmp, int vmflag)
{
	...
	int i;
#ifdef ZVM_MM_DEBUG
	printf("vmem_populate::(1)vmp->vm_nsegfree=%d\n", vmp->vm_nsegfree);
#endif
...
#ifdef ZVM_MM_DEBUG
	printf("vmem_populate::(2)vmp->vm_nsegfree=%d\n", vmp->vm_nsegfree);
#endif
	if (vmp->vm_nsegfree >= VMEM_MINFREE){
#ifdef ZVM_MM_DEBUG
	printf("vmem_populate::vmp->vm_nsegfree >= VMEM_MINFREE\n");
#endif
		return (1);
	}

#ifdef ZVM_MM_DEBUG
	    printf("vmem_populate::vmem_nosleep_lock.vmpl_thr = %u, thr_self()=%u\n",
		   vmem_nosleep_lock.vmpl_thr, thr_self());
#endif
	 ...
	 */
#ifdef ZVM_MM_DEBUG
	       printf("vmem_populate::(1)vmem_seg_arena->vm_nsegfree=%d\n", vmem_seg_arena->vm_nsegfree);
#endif
	p = vmem_alloc(vmem_seg_arena, size, vmflag & VM_UMFLAGS);
#ifdef ZVM_MM_DEBUG
	       printf("vmem_populate::(2)vmem_seg_arena->vm_nsegfree=%d\n", vmem_seg_arena->vm_nsegfree);
#endif
	if (p == NULL) {
		lp->vmpl_thr = 0;
...
#ifdef ZVM_MM_DEBUG
	    printf("vmem_populate::vmp->vm_kstat.vk_populate_fail=%d\n", vmp->vm_kstat.vk_populate_fail);
#endif
		return (0);
	}
	/*
...
#ifdef ZVM_MM_DEBUG
	printf("vmem_populate::(2)vmpl_thr=0\n", thr_self() );
#endif
...
#ifdef ZVM_MM_DEBUG
	    printf("vmem_xalloc::(1)vmp->vm_nsegfree=%d\n", vmp->vm_nsegfree);
#endif
			(void) mutex_unlock(&vmp->vm_lock);
...
#ifdef ZVM_MM_DEBUG
	    printf("vmem_xalloc::(2)vmp->vm_nsegfree=%d\n", vmp->vm_nsegfree);
#endif
			if (vaddr != NULL) {
...
vmem_alloc(vmem_t *vmp, size_t size, int vmflag)
{
...
#ifdef ZVM_MM_DEBUG
	    printf("vmem_alloc::vmp->vm_name=%s\n", vmp->vm_name);
#endif
	...
	if (vmp->vm_nsegfree >= VMEM_MINFREE || vmem_populate(vmp, vmflag)) {
		if ((size & (size - 1)) == 0)
		    ...
		    int l1 = 1UL << hb;
		    int l2 = P2ALIGN(vmp->vm_freemap, 1UL << hb);
			flist = lowbit(P2ALIGN(vmp->vm_freemap, 1UL << hb));
		}
	}
...
#ifdef ZVM_MM_DEBUG
	printf("sizeof vmem_t=%d\n", sizeof(vmem_t));
	printf("vmem_create %20s size=%8u, align=%8u, qcache_max=%8u, vmflag=%d\n", 
	       name, size, quantum, qcache_max, vmflag);
	fflush(0);
#endif

#ifdef DEBUG
	if ( !strcmp("umem_memalign", name) ){
	    int test;
	    test=1;
	}
#endif //DEBUG
...
{
#ifdef ZVM_MM_DEBUG
    int l1 = offsetof(vmem_seg_t, vs_thread);
    int l2 = sizeof (hrtime_t);
    printf("offsetof(vmem_seg_t, vs_thread)=%d, sizeof (hrtime_t)=%d", l1, l2);
#endif
	/*
...
{
#ifdef ZVM_MM_DEBUG
	    printf("vmem_lockup::vmem_nosleep_lock.vmpl_thr = %u, thr_self()=%u\n",
		   vmem_nosleep_lock.vmpl_thr, thr_self());
#endif
	vmem_t *cur;
...
vmem_release(void)
{
	...
	(void) mutex_unlock(&vmem_list_lock);
#ifdef ZVM_MM_DEBUG
	    printf("vmem_release::vmem_nosleep_lock.vmpl_thr = %u, thr_self()=%u\n",
		   vmem_nosleep_lock.vmpl_thr, thr_self());
#endif
}
...
void debug_get_vmem_internal_arena( vmem_t **vmp )  { *vmp = vmem_internal_arena; }
void debug_get_vmem_hash_arena( vmem_t **vmp )  { *vmp = vmem_hash_arena; }
void debug_get_vmem_vmem_arena( vmem_t **vmp )  { *vmp = vmem_vmem_arena; }

void debug_print_vmp(FILE* f, void* caller, const char* name, const vmem_t* vmp, int cr){
    fprintf(f, "vmem_t %s[", name);
    if ( vmp != NULL ){
	fprintf(f, "vm_name=%s, vm_id=%u, vm_mtbf=%u, vm_cflags=%d, vm_qshift=%d, ",
		vmp->vm_name, vmp->vm_id, vmp->vm_mtbf, vmp->vm_cflags, vmp->vm_qshift);
	fprintf(f, "vm_quantum=%d, vm_qcache_max=%d, ", vmp->vm_quantum, vmp->vm_qcache_max );
	fprintf(f, "vmem_alloc_t vm_source_alloc=%p, ", vmp->vm_source_alloc );
	fprintf(f, "vmem_free_t vm_source_free=%p, ", vmp->vm_source_free );
	debug_print_vmp(f, caller, "vm_source", vmp->vm_source, 0);
	debug_print_vmp(f, caller, "vm_next", vmp->vm_next, 0);
	fprintf(f, "vm_nsegfree =%d, ", vmp->vm_nsegfree );
	fprintf(f, "vmem_seg_t vm_segfree =%p, ", vmp->vm_segfree );
	fprintf(f, "vm_hash_mask =%d, vm_hash_shift=%d, vm_freemap=%d, ", 
		vmp->vm_hash_mask, vmp->vm_hash_shift, vmp->vm_freemap );
	fprintf(f, "vmem_seg_t vm_seg0 =%p, ", &vmp->vm_seg0 );
	fprintf(f, "vmem_seg_t vm_rotor =%p, ", &vmp->vm_rotor );
    }
    else{
	fprintf(f, "NULL");
    }
...
void debug_print_vmem_seg( const char* name, vmem_seg_t* seg, int p ){
    #define MAX_PADDING 4
    if ( p > MAX_PADDING ) return;
    char padding[MAX_PADDING+1];
    memset(padding, ' ', MAX_PADDING);
    padding[p]= '\0';
    if ( p > 0 )
	printf("\n");
    printf( "%s%s vmem_seg_t %p[", padding, name, seg );
    if ( seg != NULL ){
	printf( "vs_start=%x, ", seg->vs_start );
	printf( "vs_end=%x, ",   seg->vs_end );
	printf( "vs_type=%u, ",    seg->vs_type );
	printf( "vs_import=%u, ",  seg->vs_import );
	printf( "vs_depth=%u, ",   seg->vs_depth );

	debug_print_vmem_seg("vs_knext->", seg->vs_knext, p+1);
	debug_print_vmem_seg("vs_kprev->", seg->vs_kprev, p+1);
	debug_print_vmem_seg("vs_anext->", seg->vs_anext, p+1);
	debug_print_vmem_seg("vs_aprev->", seg->vs_aprev, p+1);
    }
    else{
	printf("NULL");
    }
