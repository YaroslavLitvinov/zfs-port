	...
	int cflags)		/* cache creation flags */
{
...
#ifdef ZVM_MM_DEBUG
	printf("umem_cache_create %20s, bufsize=%u, align=%u\n", name, bufsize, align);
	fflush(0);
#endif

#ifdef DEBUG
	if ( !strcmp("umem_magazine_7", name) ){
	    int h;
	    h=1;
	}
#endif
	 ...
	 */
#ifdef ZVM_MM_DEBUG
	printf("umem_cache_create sizeof umem_cache_t=%u\n", 
	       sizeof(umem_cache_t) );
	printf("umem_cache_create umem_init_thr=%u, thr_self()=%u\n", 
...
#ifdef ZVM_MM_DEBUG
	printf("umem_cache_create csize=%u, phase=%u\n", 
	       csize, phase );
#endif
