...
#ifdef DEBUG
    #include <stdio.h>
    extern void __attribute__((__no_instrument_function__))
    debug_get_vmem_list( vmem_t **vmp );

    extern void __attribute__((__no_instrument_function__))
    debug_get_vmem_internal_arena( vmem_t **vmp );

    extern void __attribute__((__no_instrument_function__))
    debug_get_vmem_hash_arena( vmem_t **vmp );

    extern void __attribute__((__no_instrument_function__))
    debug_get_vmem_vmem_arena( vmem_t **vmp );

    extern void __attribute__((__no_instrument_function__))
    debug_print_vmp(FILE* f, void* caller, const char* name, const vmem_t* vmp, int cr);
#endif
